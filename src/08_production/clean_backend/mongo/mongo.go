// Package mongo provides an abstraction layer over the official MongoDB driver.
//
// LEARNING NOTES:
// - WHY abstract? So we can mock the database in unit tests without a real MongoDB instance.
// - Every method of mongo.Client, Database, Collection is wrapped in an interface.
// - The concrete types (mongoClient, mongoDatabase, etc.) are unexported only interfaces are public.
//
// For C++ devs: This is the Adapter pattern. We wrap a third-party library behind our own interface.
// This gives us the freedom to swap implementations (e.g., for testing or switching to a different DB).
package mongo
...
// --- Public interfaces (the contracts other packages depend on) ---
...
// SingleResult wraps a single document lookup result.
type SingleResult interface {
	Decode(interface{}) error
}

// Cursor wraps a MongoDB cursor for iterating over results.
...
// Client wraps the MongoDB client lifecycle.
...
// NewClient creates a new MongoDB client (not yet connected).
